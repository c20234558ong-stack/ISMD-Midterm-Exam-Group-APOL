...
new #[Layout('layouts.app-admin')] class extends Component
{
    ...
    {
        User::role('shop owner')->whereIn('id', $this->selectedUsers)->delete();
    ...
    { 
        ...
        { 
            ...
            ->filter(fn($user) => $user->hasRole('shop owner'))
            ->pluck('id')
            ->map(fn($id) => (int) $id)
            ->toArray(); 
        } else { 
    ...
    {
        $allIds = User::role('shop owner')->pluck('id')->map(fn($id) => (int) $id)->toArray();
    ...
    {
        return User::role('shop owner')->count();
    }
